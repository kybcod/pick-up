...
export function Payment() {
    ...
    return (
        <Box
            ...
            bg="white"
        >
            <Text fontSize="2xl" mb={4}>주문 목록</Text>
            {paymentInfo.map((item) => (
                <Box key={item.id} mb={3}>
                    <Text>메뉴: {item.menuName}</Text>
                    {item.menuPrice === null || <Text>가격: {item.menuPrice.toLocaleString()} 원</Text>}
            ...
            <Flex>
                ...
                {calculateTotalAmount() === 0 ? (
                    <Button onClick={handleDirectOrders} ml={4}>주문하기</Button>
                ) : (
                    <Button onClick={onClickPayment} ml={4}>결제하기</Button>
                )}
            </Flex>
        </Box>
    );
